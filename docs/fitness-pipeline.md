...
3. 通知只包含报告周期和“打开 VikiSize 查看”的提示，不外发健康指标或分析正文。
4. Server酱失败会写入 `delivery.notification.channels.serverChan`，报告仍正常出现在小程序收件箱。
5. Server酱账号套餐可能有每日发送额度；超过额度时收件箱仍可用，但该次外部提醒会失败。
